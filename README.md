...
- **Eval scale.** 38 questions over 93 documents is enough to separate the configurations cleanly and keep the run deterministic and fast; it is not a claim about behaviour at corpus scale. The FAISS HNSW path and batched embedding cache are there so the same code scales without rewrite.
